...
const InvoiceTemplate = (data: InvoiceType) => {
  ...
  // Determine if it's an intra-state transaction (same state)
  const isIntraState = sender.state === receiver.state;
...
const calculateCGST = (isIntraState, gst, itemTotal) => {
  return isIntraState ? (gst / 2 / 100) * itemTotal : 0; // Return as a number
};

// Function to calculate SGST
const calculateSGST = (isIntraState, gst, itemTotal) => {
  return isIntraState ? (gst / 2 / 100) * itemTotal : 0; // Return as a number
};
    ...
    <InvoiceLayout data={data}>
      ...
      <div className="mt-3 overflow-x-auto">
        <div className="border border-gray-200 p-1 rounded-lg space-y-1">
          ...
          <div className="hidden sm:grid sm:grid-cols-9">
...
{details.items.map((item, index) => {
  ...
  const isInterState = details.invoiceType === "interState"; // Example type
  return (
    <div
      key={index}
      className="grid grid-cols-1 sm:grid-cols-9 gap-y-1 sm:gap-0"
    >
        ...
        <div className="border-b border-gray-300 p-2 sm:p-0">
          <p className="text-gray-800">
            {formatNumberWithCommas(
              Number(calculateCGST(!isInterState, item.gst, itemTotal))
            )}
          </p>
        </div>
      )}
      ...
      {!isInterState && (
        <div className="border-b border-gray-300 p-2 sm:p-0">
          <p className="text-gray-800">
            {formatNumberWithCommas(
              Number(calculateSGST(!isInterState, item.gst, itemTotal))
            )}
          </p>
        </div>
      )}
      ...
      {isInterState && (
        <div className="border-b border-gray-300 p-2 sm:p-0">
          <p className="text-gray-800">
            {formatNumberWithCommas(
              Number(calculateIGST(item.gst, itemTotal))
            )}
          </p>
        </div>
      )}
