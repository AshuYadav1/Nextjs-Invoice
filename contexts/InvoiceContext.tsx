...
export const InvoiceContextProvider = ({
  ...
  const uploadPdf = async () => {
    ...
    if (!invoicePdf || invoicePdf.size <= 0) {
      console.warn('No PDF to upload')
       return
    }
    // Get a reference to the storage service
    const storage = getStorage(app);

   // Create a reference to the file location in the storage
   const storageRef = ref(storage, `pdfs/invoice-${Date.now()}.pdf`); // Change to your path

    try {
       setInvoicePdfLoading(true);
       // Upload the file
       const snapshot = await uploadBytes(storageRef, invoicePdf)
  ...
  value={{
    invoicePdf,
    invoicePdfLoading, // Pass the loading state
    savedInvoices,
    pdfUrl,
    onFormSubmit,
    newInvoice,
